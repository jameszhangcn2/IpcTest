...
def per_case_logger(request, case_logger_dir):
    """
    case_logger_dir：你原有fixture，每个用例专属输出目录
    每个用例追加独立日志文件 case.log
    """
    case_log_path = os.path.join(case_logger_dir, f"case_log.log")
    fmt = logging.Formatter(
        "%(asctime)s.%(msecs)03d | %(levelname)-8s | %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S"
    )
    case_handler = logging.FileHandler(str(case_log_path), encoding="utf-8")
    case_handler.setFormatter(fmt)
    root = logging.getLogger()
    root.addHandler(case_handler)

    yield

    root.removeHandler(case_handler)
...
def setup_logging(dir_session):
    root_logger = logging.getLogger()
    root_logger.setLevel(logging.INFO)

    # 避免重复添加handler（pytest重跑会重复）
    ...
    # 2. 全局日志文件，放在项目根目录
    log_file = dir_session / "session_log.log"
...
@pytest.fixture(scope="session")
def cam_recorder():
    rec = CameraRecorder(camera_id=CAMERA_INDEX_VIDEO)
    yield rec
...
def canoe_api(kl15):
    ...
    # --------后置清理：全部用例跑完执行--------
    print("\n==== 关闭CANoe ====")
    if canoeApi.app.Measurement.Running:
        canoeApi.app.Measurement.Stop()
